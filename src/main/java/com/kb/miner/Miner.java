...
import com.kb.miner.miner.environment.Debug;
...
public final class Miner {
		...
		return new Thread() {
			...
			public void run() {
				while (!isInterrupted()) {
					...
					if (!Debug.QUIET) {
						Debug.out.println(stats.maximumHeapSize + "kB");
					}
	...
	public static final <NodeType, EdgeType> Collection<Graph<NodeType, EdgeType>> parseInput(
		...
		try {
			...
			if (Debug.VERBOSE) {
				Debug.out.println("searching on "
						+ ret.size()
			...
			pe.printStackTrace();
			Debug.err.println(pe);
			System.exit(-1);
		} catch (final IOException io) {
			Debug.err.println(io);
			System.exit(-1);
		}
	...
	private static <NodeType, EdgeType> void run(
		...
		if (!Debug.QUIET) {
			if (settings.storeEmbeddings) {
				...
				Debug.out.println("Complete run took "
						+ (settings.stats.completeTime / 1000.0) + " seconds; "
						+ "found " + fragments.size() + " fragments and "
						+ numEmbeddings + " embeddings.");
				Debug.out.println("The biggest Fragment has " + maxNodes
						+ " nodes and " + maxEdges + " edge.");
				Debug.out.println(cyclic + " cyclic Fragments are found.");
			} else {
				Debug.out.println("Complete run took "
						+ (settings.stats.completeTime / 1000.0) + " seconds; "
